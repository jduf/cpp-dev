...
void Examen::edit(){
	...
	Linux command;
	{
		...
		for(unsigned int i(0);i<test_bonus_.size();i++){ fbonus<<test_bonus_(i)<<IOFiles::endl; }

		IOFiles fdata(ndata,true,false);
		fdata<<"date="<<date_<<IOFiles::endl;
		fdata<<"bonus coefficient="<<bonus_coef_<<IOFiles::endl;
		fdata<<"bonus point="<<bonus_points_<<IOFiles::endl;
		fdata<<"questions="<<questions_<<IOFiles::endl;
...

	{
		...
		for(auto& o:options){
			...
			if(p.size()==2){
				...
				else if(p[0] == "bonus point"){ bonus_points_ = my::string2type<double>(p[1]); }
				else if(p[0] == "questions"){ 
					std::vector<std::string> q(my::string_split(p[1],' '));
					questions_.set(q.size());
					for(unsigned int i(0);i<q.size();i++){
						questions_(i) = my::string2type<unsigned int>(q[i]);
					}
				}
...
void Examen::analyse(){
	...
	unsigned int total_points(questions_.sum());
	for(unsigned int i(0);i<grades_.size();i++){
		grades_(i) = 0;
		for(unsigned int j(0);j<points_.col();j++){
			grades_(i) += points_(i,j);
		}
		grades_(i) = std::min(5*grades_(i)/(total_points-bonus_points_)+1.0,6.0);
		grades_bonus_(i) = std::min(grades_(i) + test_bonus_(i)*bonus_coef_,6.0);
		grades_final_(i) = my::round_nearest(grades_bonus_(i),2);

		if(!my::are_equal(grades_(i),1)){ 
			freq_final_(grades_final_(i)*2-2)++; 
			average_ += grades_(i);
			average_bonus_ += grades_bonus_(i);
	...
	average_ /= n_valid;
	average_bonus_ /= n_valid;
	average_final_ /= n_valid;
}

void Examen::display(){
	for(unsigned int i(0);i<class_list_.size();i++){
		std::cout<<class_list_[i]<<" "<<test_bonus_(i)<<" ";
		for(unsigned int j(0);j<points_.col();j++){
			std::cout<<points_(i,j)<<" ";
		}
		std::cout<<grades_(i)<<" "<<grades_bonus_(i)<<" "<<grades_final_(i)<<std::endl;
	}
	std::cout<<average_<<" "<<average_bonus_<<" "<<average_final_<<std::endl;
}
...
void Examen::save(){
	...
	tmp.write("Bonus points",bonus_points_);
	tmp.write("Bonus coefficient",bonus_coef_);
	tmp.write("Questions ("+my::tostring(questions_.sum())+")",questions_);
	tmp.write("Average",average_);
	tmp.write("Average (bonus)",average_bonus_);
	tmp.write("Average (final)",average_final_);
	tmp.write("Échecs",nfails_);
	tmp<<points_<<test_bonus_<<grades_<<grades_bonus_<<grades_final_;
...
void Examen::histogram(){
	...
	for(unsigned int i(0);i<freq_final_.size();i++){
		ffrequencies<<1.0+0.5*i<<" "<<freq_final_(i)<<IOFiles::endl;
	}
...
void Examen::summary(){
	...
	latex<<"\\item Nombre total de points: "<<questions_.sum()<<IOFiles::endl;
	latex<<"\\item Points de bonus: "<<bonus_points_<<IOFiles::endl;
	latex<<"\\item Coefficient bonus: "<<bonus_coef_<<IOFiles::endl;
	latex<<"\\item Échecs: "<<nfails_<<IOFiles::endl;
	latex<<"\\item Note non arrondie: $\\dfrac{5}{"<<questions_.sum()<<"-"<<bonus_points_<<"}(\\text{points})+"<<bonus_coef_<<"(\\text{bonus})+1$"<<IOFiles::endl;
	...
	latex<<"\\multicolumn{"<<points_.col()+2<<"}{r||}{Moyennes}&"<<my::round_nearest(average_,1000)<<" &"<<my::round_nearest(average_bonus_,1000)<<" &"<<my::round_nearest(average_final_,1000)<<IOFiles::endl;

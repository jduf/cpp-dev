...
int main(int argc,char* argv[]){
	...
	if(argc==2){
		...
		if ( filename != ext && filename.size() > ext.size() && filename.substr(filename.size() - ext.size()) == ext ) {
			...
			command(Linux::rst2latex(texfile,path+filename),silent);
			if(!command.status()){
				command("sed -i 's/scale=2\\.000000/width=1\\\\linewidth/g' "+texfile+".tex",silent);
				command(Linux::pdflatex("/tmp/",texfile),silent);
				command("mv " + texfile + ".pdf " + path + filename + ".pdf",silent);

...
void Latex::pdflatex(bool const& silent, bool const& clean){
	...
	if(command.status()){ std::cerr<<__PRETTY_FUNCTION__<<" : Linux::pdflatex("<<path_<<","<<filename_<<") returned an error ("<<command.status()<<")"<<std::endl; }
		...
		if(clean){
			command("rm -f " + path_ + filename_ + ".aux " + path_ + filename_ + ".log " + path_ + filename_ + ".toc " + path_ + filename_ + ".out "+ path_ + filename_ + ".tex ",silent);
			if(command.status()){ std::cerr<<__PRETTY_FUNCTION__<<" : failed to remove auxiliary tex files for '"<<path_+filename_<<"' (status:"<<WEXITSTATUS(command.status())<<")"<<std::endl; }
		}
	}
}
